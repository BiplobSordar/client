    ...
    return (
        <nav className={`p-4 ${darkMode ? 'bg-gray-900' : 'bg-white'} shadow-md border-b border-gray-200 transition duration-500`}>
            <div className="container mx-auto flex items-center justify-between">
                ...
                <div className="hidden  md:flex space-x-6 items-center">
                    <button onClick={() => navigate('/admin/dashboard')} className={`text-xl hover:text-blue-500 ${darkMode ? 'text-white' : 'text-gray-900'} transition duration-300`}>
                        <AiOutlineHome className="inline-block mr-1" /> Dashboard
                    </button>
                    < button onClick={() => navigate('/admin/courses')} className={`text-xl hover:text-blue-500 ${darkMode ? 'text-white' : 'text-gray-900'} transition duration-300`}>
                        Courses
                    </button>
            ...
            <div className={`z-10  ${darkMode ? 'bg-gray-900' : 'bg-white'} h-screen absolute top-16  left-0 transform translate ease-in-out duration-500 ${isOpen ? 'translate-x-0 ' : '-translate-x-full'}  w-full  md:hidden   `}>
                <button onClick={() => navigate('/admin/dashboard')} className={`block px-4 py-2 ${darkMode ? 'text-white' : 'text-gray-900'} hover:bg-gray-200 transition duration-300`}>
                    Dashboard
                </button>
